<template>
  <div>
    <!--标题-->
    <p class="header">
      <router-link class="btn-left" to="/">main</router-link>MeScroll所有配置项
      (详见代码)
    </p>
    <!--滑动区域-->
    <div id="mescroll" class="mescroll">
      <!--模拟的轮播,菜单 可在down.callback中配置刷新轮播数据-->
      <img class="swiper" src="~/assets/mescroll/img/swiper.jpg" />
      ...
      <ul id="dataList" class="data-list">
        <li v-for="pd in dataList" :key="pd.id">
          <img
            :imgurl="pd.pdImg"
            class="pd-img"
            src="~/assets/mescroll/img/loading.png"
          />
...
export default {
  ...
  mounted() {
    ...
    this.mescroll = new MeScroll('mescroll', {
      // 在vue的mounted生命周期初始化mescroll,确保此处配置的id能够被找到
      // 下拉刷新的所有配置项
      down: {
      ...
      setTimeout(() => {
        ...
        if (pdType === 0) {
          ...
          for (let j = 0; j < mockData.length; j++) {
            if (mockData[j].pdName.includes('奶') !== -1) {
