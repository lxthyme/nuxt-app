<template>
  <div>
    ...
    <div class="top-warp">
      <div class="head">
        <router-link class="btn-left" to="/">main</router-link>mescroll-more
      </div>
    ...
    >
      <ul id="dataList0">
        <li v-for="pd in tab0.list" :key="pd.id" class="data-li">
    ...
    >
      <ul id="dataList1">
        <li v-for="pd in tab1.list" :key="pd.id" class="data-li">
          <img :src="pd.pdImg" class="pd-img" />
          <div class="pd-name">{{ pd.pdName }}</div>
          <p class="pd-price">{{ pd.pdPrice }} 元</p>
          <p class="pd-sold">已售{{ pd.pdSold }}件</p>
    ...
    >
      <ul id="dataList2">
        <li v-for="pd in tab2.list" :key="pd.id" class="data-li">
          <img :src="pd.pdImg" class="pd-img" />
          <div class="pd-name">{{ pd.pdName }}</div>
          <p class="pd-price">{{ pd.pdPrice }} 元</p>
          <p class="pd-sold">已售{{ pd.pdSold }}件</p>
    ...
    >
      <ul id="dataList3">
        <li v-for="pd in tab3.list" :key="pd.id" class="data-li">
          <img :src="pd.pdImg" class="pd-img" />
          <div class="pd-name">{{ pd.pdName }}</div>
          <p class="pd-price">{{ pd.pdPrice }} 元</p>
          <p class="pd-sold">已售{{ pd.pdSold }}件</p>
...
export default {
        ...
        try {
          ...
          if (tabType === 0) {
            ...
            for (let n = 0; n < mockData.length; n++) {
              if (mockData[n].pdName.includes('奶粉') !== -1) {
            ...
            for (let j = 0; j < mockData.length; j++) {
              if (mockData[j].pdName.includes('面膜') !== -1) {
            ...
            for (let k = 0; k < mockData.length; k++) {
              if (mockData[k].pdName.includes('图书') !== -1) {
  ...
  beforeRouteEnter(to, from, next) {
    ...
    next((vm) => {
      const curMescroll = vm.$refs['mescroll' + vm.tabType] // 找到当前mescroll的ref,调用子组件mescroll-vue的beforeRouteEnter方法
      curMescroll && curMescroll.beforeRouteEnter() // 进入路由时,滚动到原来的列表位置,恢复回到顶部按钮和isBounce的配置
    })
  },
  beforeRouteLeave(to, from, next) {
    // 如果没有配置回到顶部按钮或isBounce,则beforeRouteLeave不用写
    const curMescroll = this.$refs['mescroll' + this.tabType] // 找到当前mescroll的ref,调用子组件mescroll-vue的beforeRouteEnter方法
